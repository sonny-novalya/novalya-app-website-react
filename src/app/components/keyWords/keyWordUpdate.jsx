...
const KeywordUpdatePop = ({containerRef}) => {
  ...
  return (
    ...
    <div ref={containerRef} className="bg-white px-6 py-5 rounded-[10px] max-w-[700px] mx-auto w-full relative max-h-[90vh] overflow-auto">
        <h3 className="text-[20px] font-medium mb-5">Update Keyword</h3>
        ...
        <div className="flex justify-end mt-4">
            <button className="bg-[#0087FF] text-white text-sm rounded-[6px] px-12 py-3 cursor-pointer" disabled={actionLoader} onClick={()=>onsubmit()}>{actionLoader?<Spin size="small" style={{color:"white"}} />:"Submit"}</button>
        </div>
    </div>
  </div>
  )
